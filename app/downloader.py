...
def download_reel_or_post(url: str) -> dict:
    ...
    try:
        ...
        if "Private" in error_msg or "Login" in error_msg:
            raise ValueError("This account is private. Cannot download without following them.")
...
def download_story(url: str) -> dict:
    ...
    if not info:
        raise ValueError("Could not parse story URL. Format: instagram.com/stories/USERNAME/STORY_ID/")
    ...
    try:
        ...
        if not target_story:
            raise ValueError(
                f"Story ID {story_id} not found. It may have expired (stories last 24h) "
                f"or the account is private."
            )
